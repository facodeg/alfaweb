    ...
    <script>
        (() => {
            ...
            const initVacationMap = () => {
                ...
                map = L.map(el, { scrollWheelZoom: false }).setView(center, first ? 12 : 6);
                ...
                markers.forEach((item) => {
                    ...
                    L.marker(position).addTo(map).bindPopup(`<strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.destination)}</span>`);
                });
