    ...
    <main class="main">
        <header class="topbar">
            ...
            <div class="top-actions">
                ...
                <a class="icon-button" href="{{ route('web.work-plans.index') }}" wire:navigate aria-label="Pesan"><i class="fa-regular fa-message"></i><span class="notif">{{ $summary['plans_today'] }}</span></a>
                <a class="icon-button" href="{{ route('web.life-schedules.index') }}" wire:navigate aria-label="Notifikasi"><i class="fa-regular fa-bell"></i><span class="notif">{{ $summary['today_schedule_count'] }}</span></a>
            </div>
        </header>
        ...
        @if ($page === 'work-targets')
            ...
            @if($workTargetChangeRequests->isNotEmpty())
                <section style="margin-top:18px" class="panel">
                    <h2>Menunggu persetujuan Anda</h2>
                    <div class="list" style="margin-top:14px">
                        @foreach($workTargetChangeRequests as $change)
                            @php
                                $proposal = $change->proposed_changes;
                            @endphp
                            <div class="item" wire:key="work-target-change-{{ $change->id }}">
                                <div class="item-main">
                                    <strong>{{ $change->requestedBy->name }} mengajukan perubahan</strong>
                                    <span>{{ $change->workTarget->title }} → {{ $proposal['title'] ?? '-' }} · {{ $proposal['progress'] ?? 0 }}% · {{ str_replace('_',' ', $proposal['status'] ?? '-') }}</span>
                                    <span>Deadline: {{ ($proposal['deadline'] ?? null) ?: 'Tanpa deadline' }}</span>
                                </div>
                                <div class="actions">
                                    <button class="btn btn-primary" wire:click="approveWorkTargetChangeRequest({{ $change->id }})" wire:confirm="Setujui perubahan target ini?">Setujui</button>
                                    <button class="btn btn-danger" wire:click="rejectWorkTargetChangeRequest({{ $change->id }})" wire:confirm="Tolak perubahan target ini?">Tolak</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <section style="margin-top:18px" class="grid-3">
                @forelse($workTargets as $target)
                    <article class="panel" wire:key="work-target-{{ $target->id }}">
                        <h2>{{ $target->title }}</h2>
                        <p>{{ $target->deadline?->format('d M Y') ?? 'Tanpa deadline' }}</p>
                        <div class="progress"><span style="width:{{ $target->progress }}%"></span></div>
                        <p style="margin-top:12px">{{ $target->progress }}% · {{ str_replace('_',' ', $target->status) }}</p>
                        <div class="actions" style="margin-top:12px">
                            <button class="btn btn-soft" wire:click="editWorkTarget({{ $target->id }})">{{ $target->user_id === $user->id ? 'Edit' : 'Ajukan edit' }}</button>
                            @if($target->user_id === $user->id)
                                <button class="btn btn-danger" wire:click="destroyWorkTarget({{ $target->id }})" wire:confirm="Hapus target pekerjaan ini?">Hapus</button>
                            @else
                                <span class="badge b-mint">Dibagikan</span>
                            @endif
                        </div>
                    </article>
                @empty
                    <div class="panel"><div class="empty">Belum ada target pekerjaan.</div></div>
                @endforelse
            </section>
        @endif
        ...
        @if ($page === 'work-plans')
            <section class="grid-2">
                <form class="form-card" wire:submit="storeWorkPlan">
                    <h2>{{ $editingWorkPlanId ? 'Edit rencana pekerjaan' : 'Rencana pekerjaan' }}</h2>
                    <p>{{ $editingWorkPlanId ? 'Ubah rencana kerja lalu simpan update.' : 'Checklist task pekerjaan seperti di Flutter.' }}</p>
                    <div class="form-grid">
                        <div class="field full">
                            <label>Judul</label>
                        ...
                        <div class="field full">
                            <div class="actions">
                                <button class="btn btn-primary" type="submit">{{ $editingWorkPlanId ? 'Update rencana' : 'Simpan rencana' }}</button>
                                @if($editingWorkPlanId)
                                    <button class="btn btn-soft" type="button" wire:click="cancelWorkPlanEdit">Batal edit</button>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="item" wire:key="work-plan-{{ $plan->id }}">
                            ...
                            <div class="actions">
                                ...
                                @if($plan->user_id === $user->id)
                                    <button class="btn btn-soft" wire:click="editWorkPlan({{ $plan->id }})">Edit</button>
                    ...
                    <article class="panel" wire:key="vacation-{{ $vacation->id }}">
                        ...
                        <div class="actions" style="margin-top:12px">
                            ...
                            @if($vacation->user_id === $user->id)
                                <button class="btn btn-soft" wire:click="editVacation({{ $vacation->id }})">Edit</button>
