    ...
    <main class="main">
        ...
        @if ($page === 'vacations')
            ...
            <section class="grid-2">
                ...
                <article class="panel map-card"><h2>Peta liburan</h2><p>Peta gratis memakai OpenStreetMap. Klik peta untuk mengisi latitude, longitude, dan URL peta otomatis.</p><div id="vacation-map" class="leaflet-map" wire:ignore></div><script type="application/json" id="vacation-map-markers">@json($vacationMarkers)</script><div class="map-help"><i class="fa-solid fa-location-crosshairs"></i><span>Klik area tujuan liburan untuk memilih titik lokasi.</span></div></article>
            </section>
            <section style="margin-top:18px" class="grid-3">@forelse($vacations as $vacation)<article class="panel" wire:key="vacation-{{ $vacation->id }}"><h2>{{ $vacation->title }}</h2><p>{{ $vacation->destination }} · {{ $vacation->start_date?->format('d M Y') ?? 'Tanggal bebas' }}@if($vacation->end_date) - {{ $vacation->end_date->format('d M Y') }}@endif</p><div class="item" style="margin-top:12px"><div class="item-main"><strong>{{ $vacation->address ?: 'Lokasi belum detail' }}</strong><span>@if($vacation->latitude && $vacation->longitude){{ $vacation->latitude }}, {{ $vacation->longitude }}@else Koordinat belum diisi @endif</span></div><span class="badge {{ $vacation->status === 'ongoing' ? 'b-mint' : ($vacation->status === 'booked' ? 'b-orange' : 'b-primary') }}">{{ $vacation->status }}</span></div><p style="margin-top:12px">Budget: {{ $vacation->budget !== null ? $money($vacation->budget) : '-' }}</p><div class="actions" style="margin-top:12px">@if($vacation->map_url)<a class="btn btn-soft" href="{{ $vacation->map_url }}" target="_blank" rel="noopener">Buka Maps</a>@endif<button class="btn btn-danger" wire:click="destroyVacation({{ $vacation->id }})" wire:confirm="Hapus rencana liburan ini?">Hapus</button></div></article>@empty<div class="panel"><div class="empty">Belum ada rencana liburan.</div></div>@endforelse</section>
        @endif
